...
<div id="content" class="detail-event detail-webinar">
		        ...
		        <div id="main" role="main">
					<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
						...
						<?php
							...
							$date_text = '<span class="date-1">'.date('M j, Y', strtotime($date)).'</span>';
							...
							echo '<div class="rsvp"><h2>'.get_field('rsvp_title').'</h2><div class="cost">'.get_field('cost').'</div><a href="'.get_field('rsvp_link').'" target="_blank" class="button">'.__('RSVP', 'igel').'</a></div>';
